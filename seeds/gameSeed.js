...
function getHighRated() {
    ...
    fetch(`https://rawg.io/api/games/?token&key=${apiKey}&metacritic=80,100&ordering=-metacritic&dates=2022-01-01,2023-01-23&exclude_additions`)
        .then(res => res.json())
        .then(data =>
...
function getGGdetails() {
    ...
    for (i = 0; i < 20; i++) {
        ...
        fetch(`https://rawg.io/api/games/${slctgame}?token&key=${apiKey}`)
            .then(res => res.json())
            .then(data => {
                // console.log(data)
